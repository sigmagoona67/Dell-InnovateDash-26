...
//
// Deno-compatible, dependency-free TypeScript port of src/lib/quietSignal.js.
// Keep this in lock-step with that source: same signals, weights, thresholds,
// combination rule, and timezone-safe local dayKey. The frontend service scores
// the seeded demo caseload with the JS version; this version scores live data
// inside the youth-ai-chat edge function. They MUST agree.
//
...
export type DriftTier = 'steady' | 'watch' | 'amber'

export interface QuietSignalMessage {
  sender: 'youth' | 'ai' | 'system' | string
  message: string
  created_at: string | number | Date
}

export interface QuietSignalSession {
  session_date: string // 'YYYY-MM-DD'
  mood_check_in?: string | null
}

export interface QuietSignalData {
  messages?: QuietSignalMessage[]
  sessions?: QuietSignalSession[]
}

export interface QuietSignalSignal {
  key: string
  label: string
  detail: string
  contribution: number
...
export interface QuietSignalResult {
  score: number
  tier: DriftTier
  signals: QuietSignalSignal[]
  window: { days: number; sessions: number; youthMessages: number }
}

export interface QuietSignalOpts {
  now?: Date | string | number
  windowDays?: number
}

// LIWC-style absolutist dictionary (the ~19 words shown to track affective severity).
const ABSOLUTIST = new Set<string>([
...
const MOOD_SCORE: Record<string, number> = { Good: 5, Okay: 4, Sad: 2, Stressed: 2, Overwhelmed: 1 }
...
const WEIGHTS: Record<string, number> = { absolutist: 25, selfFocus: 15, brevity: 20, mood: 15, withdrawal: 25 }
...
function moodToScore(label?: string | null): number | null {
  return label != null && label in MOOD_SCORE ? MOOD_SCORE[label] : null
}
 ...
 */
export function computeQuietSignal(data: QuietSignalData, opts: QuietSignalOpts = {}): QuietSignalResult {
  const now = opts.now ? new Date(opts.now) : new Date()
  const windowDays = opts.windowDays ?? 14
  const windowStart = startOfDay(now).getTime() - (windowDays - 1) * DAY_MS
  const nowMs = now.getTime()

  const inWindow = (ts: string | number | Date): boolean => {
  ...
  const moodPoints = sessions
    ...
    .filter((p): p is { key: string; y: number } => p.y != null)
  ...
  let tier: DriftTier = 'steady'
  if (score >= 55) tier = 'amber'
  else if (score >= 30) tier = 'watch'

  // ---- Build the explainable "why" (only fired signals surface) ----
  const signals: QuietSignalSignal[] = []
  const pushSignal = (key: string, label: string, contribution: number, detail: string): void => {
 ...
 */
export function buildDriftSeries(data: QuietSignalData, opts: { now?: Date | string | number; days?: number; windowDays?: number } = {}): number[] {
  const now = opts.now ? new Date(opts.now) : new Date()
...
// Templated mood message pattern (e.g. "I'm feeling Sad today.") inserted by the
// recordMood action. These are NOT genuine youth writing and must be excluded
// when gathering youth messages for scoring — otherwise a silent check-in (mood
// logged, nothing written) would always look like the youth wrote something, and
// the "opened, didn't write" withdrawal signal would never fire.
export const TEMPLATED_MOOD_RE = /^I'?m feeling .+ today\.?$/

/** True if this message is a templated mood line or a system message (skip for scoring). */
export function isTemplatedYouthMood(msg: { sender?: string; message?: string }): boolean {
  if (!msg) return false
  if (msg.sender === 'system') return true
  return TEMPLATED_MOOD_RE.test(String(msg.message || '').trim())
}
