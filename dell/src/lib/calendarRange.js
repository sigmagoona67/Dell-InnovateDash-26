...
export function getWindowLabel(window = getCalendarWindow()) {
  const minLabel = new Date(window.minYear, window.minMonth - 1, 1).toLocaleString('default', {
    month: 'long',
    year: 'numeric',
  })
  const maxLabel = new Date(window.maxYear, window.maxMonth - 1, 1).toLocaleString('default', {
    month: 'long',
    year: 'numeric',
  })
